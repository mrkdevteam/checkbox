...
if ( ! class_exists( 'MRKV_CHECKBOX_AUTOCREATE' ) ) {
    ...
    class MRKV_CHECKBOX_AUTOCREATE {
        ...
        /**
         * Get the correct cashbox ID based on settings.
         */
        protected function get_cashbox_id( $payment_id, $settings ) {
            if ( empty( $settings ) ) {
                $legacy_names = get_option( 'ppo_autocreate_checkbox_name', [] );
                return $legacy_names[ $payment_id ] ?? 'default';
            }
            return $settings['automation']['payments'][ $payment_id ]['register'] ?? 'default';
        }
