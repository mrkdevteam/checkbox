...
add_action('checkbox_open_shift', 'mrkv_checkbox_connect');
...
if (! function_exists('mrkv_checkbox_create_receipt')) {
    ...
    function mrkv_checkbox_create_receipt($api, $order)
    {
        ...
        if (isset($receipt['error'])) {
            ...
            $result['message'] = sprintf('%s: %d.%s: %s.', __('Код помилки', 'checkbox'), $receipt['error']['code'], __('Повідомлення:', 'checkbox'), $receipt['error']['message']);
        ...
        if (isset($receipt['id'])) {
            ...
            update_post_meta($order->get_id(), 'receipt_url', sprintf('https://check.checkbox.ua/%s', $receipt['id']));
            $result['success'] = true;
    ...
    function mrkv_checkbox_disconnect()
    {
        ...
        if ($login && $password && $cashbox_key) {
            ...
            if (isset($shift['id'])) {
                ...
                $logger->info(__('Зміна успішно закрита', 'checkbox'));
                ...
                $logger->error(sprintf('%s %s', __('Під час закриття зміни виникла помилка. Повідомлення:', 'checkbox'), $shift['message']));
...
if (! function_exists('mrkv_checkbox_show_plugin_admin_page')) {
    ...
    function mrkv_checkbox_show_plugin_admin_page()
    {
        ...
        <style>
            ...
            span.tooltip {
                background-image: url('<?php echo plugin_dir_url(__FILE__) ?>assets/img/tooltip-icon.svg');
            }
                ...
                <table class="form-table">
                    ...
                    <tr class="order-statuses" valign="top" style="<?php echo ( 1 == get_option('ppo_autocreate') ) ? '' : 'display: none;'; ?>">
                        ...
                        <td>
                            ...
                            ?>
                            <select class="chosen order-statuses" name="ppo_autocreate_receipt_order_statuses[]" data-placeholder="<?php _e('Виберіть статуси замовлення', 'checkbox') ?>" multiple>
                                <?php
                                if (! empty($all_order_statuses)) :
                                    foreach ($all_order_statuses as $k => $v) :
                                        $k = str_replace('wc-', '', $k);
                                        if (empty($autocreate_receipt_statuses)) :
                                            ?>
                                        <option value="<?php echo $k; ?>"><?php echo $v; ?></option>
                                            <?php
                                        else :
                                            ?>
                                        <option value="<?php echo $k; ?>" <?php echo ( in_array($k, $autocreate_receipt_statuses) ) ? 'selected' : ''; ?>><?php echo $v; ?></option>
                                            <?php
                                        endif;
                                    endforeach;
                                else :
...
if (! function_exists('mrkv_checkbox_styles_and_scripts')) {
    ...
    function mrkv_checkbox_styles_and_scripts($hook)
    {
        ...
        wp_enqueue_style('checkbox', plugin_dir_url(__FILE__) . 'assets/css/checkbox.css', array(), CHECKBOX_VERSION);
        wp_enqueue_script('checkbox', plugin_dir_url(__FILE__) . 'assets/js/checkbox.js', array('jquery'), CHECKBOX_VERSION, true);
        // microtip
